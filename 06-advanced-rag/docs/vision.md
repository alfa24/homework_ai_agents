...
- **langchain / langchain-openai / langchain-community** - RAG-пайплайн для справочного ассистента в LCEL-стиле (query transformation, retrievers, промпт-цепочки)
...
- **rank-bm25** - BM25-поиск через `BM25Retriever` для точных совпадений терминов
- **sentence-transformers** - локальные HuggingFace embeddings и cross-encoder reranker
- **langsmith** - трейсинг RAG-запросов (автоматически через переменные окружения `LANGSMITH_*`)
- **ragas** (>=0.2.0) - evaluation-фреймворк для оценки качества RAG (faithfulness, answer_relevancy, context_precision и др.)
- **datasets** - загрузка и работа с датасетами (зависимость RAGAS)
...
│   ├── rag_service.py             # RAG-сервис: query transform + semantic/hybrid retrieval + reranking
...
5. **config.py** - конфигурация
   ...
   - Поля: `telegram_token`, `openai_api_key`, `openai_base_url`, `model_text`, `model_image`, `model_audio`, `system_prompt`, `proxy_url`, `model_chat_rag`, `model_embeddings`, `data_dir`, `chunk_size`, `chunk_overlap`, настройки retrieval-режима, semantic, BM25, hybrid, reranker и RAGAS
...
6. **rag_service.py** - RAG-ассистент
   - Владеет `InMemoryVectorStore`, BM25 retriever, опциональным cross-encoder reranker и LCEL-цепочками LangChain
   - `reindex()` - загружает документы через `document_loader`, строит индекс заново
   - `answer(chat_id, question)` - query transformation с учётом истории → выбранный retrieval-режим → опциональный reranking → финальный ответ LLM по контексту
   - `status()` - количество документов/чанков, режим retrieval и время последней индексации
   - Использует `ChatOpenAI`, embeddings-провайдер из конфига и cross-encoder модель из env
...
Telegram → handlers.py (команда /ask) → RagService.answer(chat_id, question) →
  ...
  выбранный retrieval mode:
    semantic: InMemoryVectorStore.as_retriever(k=SEMANTIC_RETRIEVER_K)
    hybrid: EnsembleRetriever(semantic + BM25)
    hybrid_rerank: EnsembleRetriever(semantic + BM25) → CrossEncoder rerank top-K
...
bot.py (startup) / handlers.py (/index) → RagService.reindex() →
  ...
  embeddings provider (openai/huggingface) → InMemoryVectorStore.from_documents(...) + BM25Retriever.from_documents(...) → готовые retrievers в памяти
```
...
RAG-ассистент использует LangChain в LCEL-стиле поверх OpenRouter-compatible chat model (`ChatOpenAI` с `base_url=OPENAI_BASE_URL` и `api_key=OPENAI_API_KEY`). Embeddings и RAGAS embeddings выбираются отдельно: `openai` через `OpenAIEmbeddings` или `huggingface` через `HuggingFaceEmbeddings` / `sentence-transformers`.
...
- Итоговый список `Document` индексируется в `InMemoryVectorStore.from_documents(...)` для semantic retrieval и одновременно передаётся в `BM25Retriever.from_documents(...)` для keyword retrieval.

**Режимы retrieval (`RAG_RETRIEVAL_MODE`):**
- `semantic` — только semantic retriever из `InMemoryVectorStore`, top-K задаётся `SEMANTIC_RETRIEVER_K`.
- `hybrid` — semantic retriever + `BM25Retriever` через `EnsembleRetriever`; веса задаются `HYBRID_SEMANTIC_WEIGHT` и `HYBRID_BM25_WEIGHT`, candidate K — отдельными настройками semantic/BM25/hybrid.
- `hybrid_rerank` — сначала hybrid retrieval, затем `CrossEncoder` из `sentence-transformers` сортирует кандидатов и оставляет `RERANKER_TOP_K` документов для ответа.

**Пайплайн ответа (`RagService.answer`):**
1. `rag_query_transform_chain` (см. `docs/references/naive-rag.ipynb`): LLM берёт историю диалога (`list[BaseMessage]` из `RagConversationStore`) и текущий вопрос, переписывает его в самодостаточный поисковый запрос. Этот этап сохраняется для всех режимов Advanced RAG.
2. LCEL-цепочка выбирает retriever по `RAG_RETRIEVAL_MODE`: semantic, hybrid или hybrid + reranker (см. `docs/references/advanced-hybrid-rag.ipynb` Part 1 и Part 2).
...
- `EMBEDDINGS_PROVIDER=openai` — использовать `OpenAIEmbeddings(model=MODEL_EMBEDDINGS, base_url=OPENAI_BASE_URL, api_key=OPENAI_API_KEY)`.
- `EMBEDDINGS_PROVIDER=huggingface` — использовать `HuggingFaceEmbeddings(model_name=MODEL_EMBEDDINGS)`, по умолчанию CPU и нормализация embeddings.
- `RAGAS_EMBEDDINGS_PROVIDER` и `MODEL_RAGAS_EMBEDDINGS` задают embeddings отдельно для RAGAS evaluation, чтобы не связывать качество retrieval и оценщика.
- Все названия моделей (`MODEL_CHAT_RAG`, `MODEL_EMBEDDINGS`, `MODEL_RAGAS_LLM`, `MODEL_RAGAS_EMBEDDINGS`, `MODEL_CROSS_ENCODER`) задаются через env.
...
- Включается переменной `SHOW_SOURCES=true` в `.env`.
- RAG-цепочка возвращает использованные документы вместе с ответом.
- Формат: `📚 Источники: filename.pdf (стр. 1, 3, 5)` — добавляется к ответу бота.

**LangSmith-трейсинг:**
- Автоматический трейсинг всех LangChain-вызовов через переменные окружения `LANGCHAIN_TRACING_V2`, `LANGCHAIN_API_KEY`, `LANGCHAIN_PROJECT`.
- Код менять не нужно — LangChain подхватывает конфигурацию автоматически.

**Синтез QA-датасетов (`src/dataset_synthesizer.py`):**
- Выборка по 2 чанка из каждого PDF в `data/`.
- LLM генерирует вопрос и ответ по каждому чанку.
- Поддержка загрузки готовых Q&A из JSON-файлов.
- Сохранение в `datasets/05-rag-qa-dataset.json`, загрузка в LangSmith с проверкой на дубликаты.
- Команды `make dataset` и `make dataset-upload`.

**RAGAS Evaluation (`src/evaluation.py`):**
- Команда бота `/evaluate_dataset` запускает полный цикл оценки.
- 6 метрик: `faithfulness`, `answer_relevancy`, `answer_correctness`, `answer_similarity`, `context_recall`, `context_precision`.
- Модели для RAGAS задаются через `MODEL_RAGAS_LLM`, `RAGAS_EMBEDDINGS_PROVIDER` и `MODEL_RAGAS_EMBEDDINGS` в `.env`.
- Evaluation прогоняет текущий `RAG_RETRIEVAL_MODE`, чтобы можно было сравнивать `semantic`, `hybrid` и `hybrid_rerank` без изменения кода.
...
# RAGAS
MODEL_RAGAS_LLM=openai/gpt-oss-20b:free
...
# Advanced RAG: semantic | hybrid | hybrid_rerank
RAG_RETRIEVAL_MODE=semantic
EMBEDDINGS_PROVIDER=openai
MODEL_EMBEDDINGS=openai/text-embedding-3-small
SEMANTIC_RETRIEVER_K=4
...
**Что НЕ логируем:**
- Детальные трейсы успешных операций (трейсинг LangChain-вызовов делегирован LangSmith)

**Вывод:** stdout/stderr (консоль) + LangSmith (при наличии `LANGSMITH_*` переменных)

**Принципы:**
- Без внешних библиотек (structlog и т.п.)
- Без файлов, ротации логов
- Трейсинг RAG-запросов — через LangSmith (автоматически, без изменения кода)
- Простой текстовый формат для локальных логов
