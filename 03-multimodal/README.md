...
- ✅ Обработка голосовых сообщений (транскрибация через whisper-1)
...
- `MODEL_AUDIO` - модель для транскрибации голосовых сообщений (по умолчанию `openai/whisper-1`, работает только через OpenRouter/OpenAI)
...
- Голосовые сообщения обрабатываются только через OpenRouter/OpenAI (Ollama не поддерживает endpoint транскрибации аудио)
